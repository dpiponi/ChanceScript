...
{
  void ddist<state> strategy(const state &);
};
    ...
    return roll(20) >> [=](int to_hit) {
      if (to_hit >= 11) {
        return roll(6) >> [=](int damage) {
          ...
          damaged_player.hit_points =
              std::max(0, damaged_player.hit_points - damage);
          return certainly(state{damaged_player, monster});
        };
      } else {
        return certainly(s);
      }
...
void test6a() {
  ...
  ddist r = iterate_matrix_i(
      s, [](const state s) { return player_move(s) >> monster_move; }, 1000);

  for (auto z : r.pdf) {
    std::cout << z.value.player.hit_points << ' ' << z.value.monster.hit_points
              << ' ' << z.prob << std::endl;
  }
}
